...
export const Filter: FC<FilterProps> = ({applyFilter, setFilters, filters, data}) => {
    ...
    useEffect(() => {
        if (data.length) {
            setDataSources(_.uniqBy<AdData>(data, 'dataSource').map(({dataSource}) => dataSource).filter(v => v));
            setCampaigns(_.uniqBy<AdData>(data, 'campaign').map(({campaign}) => campaign).filter(v => v));
        }
    }, [data])

    const createOptions = useMemo(() => (data: string[]): Option[] => {
        return [
            AllOption,
            ...data.map((item) => {
                return {
